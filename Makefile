...
.PHONY: all build vet fmt-check test unit-test integration-test examples-build install uninstall clean fetch_meta gen_meta gitleaks
...
# gen_meta compiles meta_data.json into static Go (meta_data_gen.go), consumed
# under -tags larkmeta for a zero-parse / zero-alloc startup. Must run after
# fetch_meta (needs meta_data.json) and before any -tags larkmeta build.
gen_meta: fetch_meta
	go run internal/registry/metastatic/gen.go

build: gen_meta
	go build -tags larkmeta -trimpath -ldflags "$(LDFLAGS)" -o $(BINARY) .
