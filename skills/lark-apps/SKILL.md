...
| 插件集成 — 用户要实现以下能力时必须走插件链路：AI生文/AI生图/AI翻译/AI摘要/AI分类/图片理解/图片识别/图片抠图/图片对比/图生图/语音识别/语音合成/文档解析/网页抓取/文本转JSON；或提到 Plugin/PluginInstance/Capability/插件安装/卸载/创建实例 | 先读插件入口 Skill 确认项目上下文，再按意图路由到 CRUD 链路 | [`lark-apps-plugin.md`](references/lark-apps-plugin.md) |
