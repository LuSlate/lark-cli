...
- 用户想把文档回滚到某个 `revision_id` 或某一时刻 → 先读 [`lark-doc-history.md`](references/lark-doc-history.md)，执行 `docs +history-list`，根据列表里的 `revision_id` 或 `edit_time` 匹配到对应 `history_version_id`，再用 `docs +history-revert --history-version-id <id>` 回滚；不要把 `revision_id` 直接传给回滚命令
