...
| `skills/ppt-master/SKILL.md` Eight Confirmations | 生成前确认设计参数；SVGlide 默认改为机器审查和 hash 绑定，不要求用户确认 | `strategy_review` + `palette_review` + `selection_review` + `plan_bundle_review`; optional `confirm_plan` only for compatibility | adapt |
...
| `SKILL.md` Executor | 真正生成 SVG 的阶段必须在计划审查和资产后，不能提前写 SVG | `generate_svg` stage | yes |
