...
5. **列宽 + 行高 + 自动换行**：详细规则见 `lark-sheets-range-operations` 的「写入后列宽自适应」章节（按最长字符数扩列宽 / 长文本设置 `wrap=true` + 调高行高 / 长数字设置 `number_format` 防科学计数法）
...
> 具体采样调用与工作流见 `lark-sheets-read-data` 的「格式采样工作流（新增行时继承原有格式）」章节。
