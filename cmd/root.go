...
AGENT QUICKSTART (driving this as an agent? start here):
    Check session:    lark-cli auth status               # not logged in? lark-cli auth login
    Find a domain:    lark-cli skills list                # JSON: every domain skill + when to use it
    Orient a domain:  lark-cli skills read lark-<domain>  # concepts + which command to use
